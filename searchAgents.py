...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState, costFn=lambda x: 1):
        ...
        "*** YOUR CODE HERE ***"
        self.cornersLeft = tuple(corner for corner in self.corners if corner is not self.startingPosition)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if next in cornersLeft:
                    cornersLeft = tuple(corner for corner in cornersLeft if corner != next)
                nextState = (next, cornersLeft)
                cost = 1
                successors.append((nextState, action, cost))
...
def cornersHeuristic(state, problem):
    ...
    # pairs = ((a, b) for a in state[1] for b in state[1])
    heuristic = 0
    curPos = state[0]
